...
| `error_boundary` | Reactive | Wrap a render function so an exception inside it shows a visible error card scoped to that subtree, instead of blanking the tree. Outside a `window` or `modal` the card comes with its own title bar and close button, so a failed render is always dismissible. First two args mirror `component()` (`renderer, props`); kwargs style the error card (`background_color`, `padding`, `border_radius`, ...) or override pieces (`label`, `title_color`, `message_color`, `code_props`, `fallback`). | `error_boundary(my_view, props, background_color="111", label="MyView")` |
