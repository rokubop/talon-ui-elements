...
| -- | -- | -- |
| `user.ui_elements()` | `elements: Union[str, List[str]]` | **Primary method for building UIs.** Get UI elements by name. Returns tuple or single element. Example: `div, text = actions.user.ui_elements(["div", "text"])`. See [Elements documentation](./elements.md)<br><br>**Root elements:** `screen`, `active_window`<br>**Standard elements:** `window`, `div`, `text`, `button`, `input_text`, `textarea`, `select`, `checkbox`, `link`, `icon`, `table`, `tr`, `th`, `td`, `cursor`<br>**SVG elements:** `svg`, `path`, `rect`, `circle`, `line`, `polyline`, `polygon`<br>**Utility elements:** `state`, `ref`, `effect`, `component`, `style` |
...
| `user.ui_elements_get_input_value()` | `id: str` | Get the current value of an `input_text` or `textarea` element by ID |
