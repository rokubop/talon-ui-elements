...
class NodeTable(NodeContainer):
    ...
    def v2_reconcile_row_heights(self, available_size: Size2d = None):
        ...
        row_heights comes from unwrapped text at measure time. A wrapped cell
        grows past it, and the layout is column-major, so without this the
        cell pushes its own column down and every row below desyncs.
        """
        if not self.rows:
            return
